...
//메뉴그룹 수정
	...
	//옵션그룹 수정
	$(".updateOptionGroup").click(function() {
		...
		Swal.fire({
			...
			//입력값이 있을 경우
  		    if (result.value) {
  		    	$.ajax({
  		    	    type: "POST",
  		    	    url: "/updateOptionGroup.store",
  		    	    data: JSON.stringify({ "og_code": og_codeforUpdate , "og_name": result.value}), //{md_code랑 변경할 이름으로 가져옴}
  		    	    dataType: "json",
  		    	    contentType: "application/json",
  		    	    success: function (data) {
  		    	        if (data == 1) {
...
// 			  title: "삭제하시겠습니까??",
	...
	//온셥그룹 삭제
	$(".deletingOptionGroup").click(function() {
		var og_codeforDelete = $(this).attr("value");
		var fordelete = $(this).attr("fordelete"); //a 의 id
		var fordelete2 = $(this).attr("fordelete2"); //ul 의 id
		var fordelete3 = $(this).attr("fordelete3"); //button의 id
		//삭제 확인
...
// 			  title: "삭제하시겠습니까??",
			  ...
			  if (result.isConfirmed) {
			 		$.ajax({
	 	 		  type: "POST",
	 	           url: "/deleteOptionGroup.store",
	 	          data: JSON.stringify({"og_code":og_codeforDelete}),
	 	           dataType: "json",
	 	          contentType: "application/json",
	 	          success:function(data){
	 	        	  if(data==1){
