...
public class AdminUserController {
	...
	public void userExcelDownload(Model model, HttpServletResponse response, HttpServletRequest request)
			throws Exception {
		...
		for (int i = 0; i < userList.size(); i++) {
			...
			if (userList.get(i).getU_type().equals("normal")) {
				userList.get(i).setU_type("일반");
			} else if (userList.get(i).getU_type().equals("Google")) {
				userList.get(i).setU_type("구글");
			} else if (userList.get(i).getU_type().equals("KaKao")) {
				userList.get(i).setU_type("카카오");
			} else if (userList.get(i).getU_type().equals("Naver")) {
				userList.get(i).setU_type("네이버");
			} else {
				userList.get(i).setU_type("부잉");
			}
			...
			if (userList.get(i).isU_sms_usable())
			...
			if (userList.get(i).isU_email_usable())
				dataCell.setCellValue("O");
			else
				dataCell.setCellValue("X");
			dataCell.setCellStyle(d_style1);
			...
			if (userList.get(i).isU_status())
				dataCell.setCellValue("활동");
			else
				dataCell.setCellValue("비활동");
			dataCell.setCellStyle(d_style1);
			cellNum++;
